...
// Local calendar date (Y/M/D) of a unix timestamp in the given IANA timezone.
// Without this, a Sunday-23:59 Bogota end-of-week (= Monday UTC) would be
// labelled as the *next* ISO week. tz defaults to UTC for back-compat.
function localYMD(unixSec, tz = 'UTC') {
...
export function weekLabel(window, style, tz = 'UTC') {
  const { y, mo, d } = localYMD(window.latest, tz);
  if (style === 'date') {
    return `wk ending ${y}-${String(mo).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
  }
